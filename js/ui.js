...
export class UI {
  ...
  mostrar(eventos) {
    ...
    eventos.forEach(evento => {
      this._crearCardEvento(evento);
    });
  }

  agregarEvento(evento) {
    const mensaje = this.contenedor.querySelector('p');
    if (mensaje && mensaje.textContent.includes('No hay eventos registrados')) {
      mensaje.remove();
    }
    this._crearCardEvento(evento);
  }

  _crearCardEvento(evento) {
...
// Función para generar factura con jsPDF
